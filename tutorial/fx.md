...
Стоит сразу оговориться, что эквалайзер - это технология Web Audio API и нужно понимать то, как данная библиотека работает с этим самым API. Есть несколько особенностей, которые стоит учитывать проектируя собственные фильтры препроцессора (да и просто для использования эквалайзера не плохо знать это).
  ...
  4. **Важно**. Однажды включенный режим Web Audio API отключить нельзя! Когда вызывается метод `YandexAudio#toggleWebAudioAPI(false)` граф просто максимально сокращается. Из него удаляется препроцессор, элемент `GainNode` и управление громкостью возвращается элементу `audio`.
