...
```AnalyzerNode``` возвращает 2 набора данных - частотные характеристики и форму волны. Стоит сразу отметить что оба набора данных представлены набором вещественных чисел, а не комплексных. Для формы волны данный факт не вызывает удивления - это всего лишь временный буфер сигнала над которым производится преобразование, а цифровой сигнал не имеет мнимой части. Частотные же характеристики, которые получаются после БПФ должны иметь мнимую часть. В данном случае возвращаемые значения - это модули данных чисел, т.е. возвращаемые значения в данном случае - это амплитуды косинусоид.

**УТОЧНИТЬ ПРО РАЗМЕР БУФЕРА И РАЗМЕР ВЫХОДНЫХ ДАННЫХ**

Выше была рассмотрена интерпретация значений частотных характеристик в системе координат преобразования, теперь стоит рассмотреть интерпретацию в системе координат цифрового сигнала. Чему равна реальная частота каждого из аргументов? Это зависит от 2х параметров: частоты дискретизации самого цифрового сигнала и размера буфера преобразования. Считать удобнее от самой высокой частоты, т.е. от самого последнего аргумента. Эта частота будет частотой Найквиста. **ПЕРЕПРОВЕРИТЬ** Зависимость частоты от номера аргумента получается такая: D * (n - 1) / B, где D - частота дискретизации сигнала, B - размер буфера, n - номер аргумента. 

**УТОЧНИТЬ ЕДИНИЦЫ В КОТОРЫХ ВЫВОДИТСЯ РЕЗУЛЬТАТ И КАК ПРОИСХОДИТ ПЕРЕМАСШТАБИРОВАНИЕ**
Вернёмся ещё раз к самим значениям частотных характеристик. Как уже было сказанно ранее ```AnalyzerNode``` выдаёт не сырые результаты БПФ, а модули этих результатов. Но на самом деле это не всё что происходит. Сначала вычисляются модули из значений БПФ, затем они переводятся в децибелы, затем производится перенормирование результатов в указанных границах (min, max **ПОСТАВИТЬ ССЫЛКИ**). В конце происходит усреднение значений с значениями предыдущих расчётов - это скорее косметическая операция, которая делает переходы между 2 идущими последовательно порциями данных более плавными, она задаётся коэффициентом ... **ВСТАВИТЬ ССЫЛКУ** и работает примерно так: r = a * (1 - k) + (a1 * k)).

http://jsfiddle.net/pvktoo8a/
