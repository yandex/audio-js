...
Для простоты назовём сигнал передающийся между 2 элементами ```AudioNode``` потоком. У каждого элемента есть какое-то количество входов и выходов потока. Не стоит путать количество входов с количеством подключенных потоков. К любому входу можно подключить неограниченное количество потоков. Аналогично дела обстоят с выходами - любой выход можно подключить к любому количеству нод. Большинство элементов имеет по 1 входу и выходу. Источники сигнала не имеют входов, а выводящие элементы (которые воспроизводят звук или создают сетевой поток), не имеют выходов. 
...
Стоит отдельно упомянуть про работу с многоканальным звуком. Каждый элемент ```AudioNode``` имеет свои параметры указывающие на количество каналов, с которыми он работает и то, каким образом данные каналы следует интерпретировать в случае, если соединяются 2 элемента с разным количеством каналов.
  ...
  - ```AudioNode#channelInterpretation``` - ([man](https://developer.mozilla.org/en-US/docs/Web/API/AudioNode/channelInterpretation)) самая интересная настройка. Она отвечает за то, что будет происходить если соединить 2 элемента у которых разное количество каналов. Параметр может принимать одно из значений
    ...
    * ```"discrete"``` - смешение и разделение каналов отключено. "Лишние" каналы отбрасываются, "недостающие" - просто содержат нулевой сигнал
...
Для большинства элементов ```AudioNode#channelCountMode``` выставлен в значение ```"max"```, так что при работе с ними можно не обращать внимания на все эти настройки. Рассмотрим отдельно элементы в которых используются другие значения.
  ...
  - ```PannerNode``` - для этого элемента существует 2 ограничения
    - значение ```channelCountMode``` может быть только ```"clamped-max"``` или ```"explicit"```
    - значение ```channelCount``` может быть только 1 или 2
  - ```ConvolverNode``` - для этого элемента значения по-умолчанию ```channelCountMode = "clamped-max"``` и ```channelCount = 2```, но их можно произвольно менять. Однако есть ограничение на количество каналов буфера, используемого при рассчёте свёртки - их может быть 1, 2 или 4 ([git](https://github.com/WebKit/webkit/blob/20f480a542d431da0d4ffe84347b498bdd614bb8/Source/WebCore/Modules/webaudio/ConvolverNode.cpp#L134)).
  - ```AudioDestinationNode``` - для этого элемента значения по-умолчанию ```channelCountMode = "explicit"``` и ```channelCount = 2```. Можно выставить любое значение для автоматического рассчёта количества каналов, но для настройки ```channelCount``` есть ограничение в количество каналов, которое поддерживается системой. Это значение можно узнать из ```AudioDestinationNode#maxChannelCount```. При установке ```channelCountMode = "max"``` это интерпретируется как ```channelCountMode = "clamped-max";``` и ```channelCount = AudioDestinationNode#maxChannelCount```.
  - ```AnalyserNode``` - по умолчанию имеет ровно 1 канал. Однако можно выставить любое количество каналов. При этом при формировании буфера для БПФ будет рассчитано среднее-арифмитическое значений из всех доступных каналов.
...
Данный элемент представляет из себя источник звука в 3х-мерном пространстве. 

Фабрика ```AudioContext#createPanner``` ([man](https://developer.mozilla.org/en-US/docs/Web/API/AudioContext/createPanner)) не принимает никаких параметров. Все настройки делаются после создания элемента (например ```pannerNode.distanceModel = "inverse"```).

Данный элемент имеет множество настроек:
  ...
  - ```PannerNode#distanceModel``` - модель используемая для расчёта эффекта затухания звука при удалении от источника. Может принимать одно из 3х значений: ```"linear"```, ```"inverse"```, ```"exponential"```. Иллюстрацию всех моделей можно посмотреть в данной [демонстрации](https://www.desmos.com/calculator/ro4d3bp7m5) (k = rolloffFactor, d0 = refDistance, dl = maxDistance):
    * ```linear``` - громкость пропорциональна расстоянию до сферы ```maxDistance``` (```1 - rolloffFactor * (distance - refDistance) / (maxDistance - refDistance)```)
    * ```inverse``` - громкость обратнопропорциональна расстоянию до сферы ```refDistance``` (```refDistance / (refDistance + rolloffFactor * (distance - refDistance))```)
    * ```exponential``` - громкость обратнопропорциональна степени расстояния до сферы ```refDistance``` (```Math.pow(refDistance / distance, rolloffFactor)```)
  - ```PannerNode#coneInnerAngle``` - каждый источник воспроизведения является не всенаправленным, а коническим. Данный параметр указывает угол в градусах при вершине внутреннего конуса звучания. Внутри этого конуса не происходит снижения громкости звука. Значение по-умолчанию 360 градусов делает источник по сути всенаправленным.
...
Кроме данных параметров каждый источник должен иметь некую позицию и ориентацию в пространстве. Эти параметры задаются с помощью методов
  ...
  - ```PannerNode#setVelocity(x, y, z)``` - устанавливает вектор движения в декартовой системе координат. Значения указываются в метрах в секунду. Требуется для рассчёта [эффекта Доплера](https://ru.wikipedia.org/wiki/Эффект_Доплера)
...
В отличие от ```PannerNode``` данный объект является свойством аудио-контекста и в рамках одного контекста может существовать лишь в одном экземпляре. Данный объект имеет значительно меньше настроек, чем ```PannerNode```. Слушатель может иметь только позицию и ориентацию, но не может иметь скорости движения.
