...
Почему-то нигде в документации не описано в каком формате методы ```AnalyserNode#getFloatFrequencyData``` и ```AnalyserNode#getByteFrequencyData``` возвращают данные, сказано лишь, что эти данные получаются из БПФ. Нам известно, что БПФ возвращает список комплексных чисел. Первой мыслью было, что данные методы возвращают просто модули этих значений, но это оказалось не так. Возвращаемые данные условно разделены на 2 равных части. Первая половина данных содержит модули (т.е. значения амплитуды), а вторая часть - аргументы (т.е. значения фазовых сдвигов). И это ещё не всё. Для метода ```AnalyserNode#getFloatFrequencyData``` данные об амплитуде задаются по шкале dBFS, а для метода ```AnalyserNode#getByteFrequencyData``` эта шкала ещё и перенормируется с учётом параметров ```AnalyserNode#minDecibels``` и ```AnalyserNode.maxDecibels```. Не очевидно ещё вот что: размер данных о частотных характеристиках в 2 раза меньше, чем размер буфера (```AnalyserNode.fftSize```), а БПФ должно выдавать данные соответствующие размеру буфера. Отсюда берутся 3 варианта - данные БПФ обрезаются до половины, берётся первая половина буфера для анализа или берётся вторая половина буфера для анализа. Правильным является предположение о том, что половина данных отбрасывается. Чуть позже объясню почему для анализа она действительно мало-интересна.

С методами ```AnalyserNode#getFloatTimeDomainData``` и ```AnalyserNode#getByteTimeDomainData``` всё несколько проще - оба метода просто возвращают значения временного буфера. Значения первого идут в диапозоне от -1 до 1, значения второго - от 0 до 255. **ПЕРЕПРОВЕРИТЬ, ЧТО ЗНАЧЕНИЯ БЕЗЗНАКОВЫЕ**
